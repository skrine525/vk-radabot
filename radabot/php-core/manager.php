...
function manager_panel_control($finput)
{
	...
	if ($command == "создать") {
		...
		$bulk = new MongoDB\Driver\BulkWrite;
		$bulk->update(['_id' => $db->getDocumentID()], ['$set' => ["chat_settings.user_panels.id{$data->object->from_id}" => $user_panel]]);
		$db->executeBulkWrite($bulk);
		...
		if (count($user_panel["elements"]) > 0) {
			...
			$id = 1;
			foreach ($user_panel["elements"] as $element) {
				$msg .= "\n{$id}. {$element["name"]}: [{$element["command"]}]";
				$id++;
			}
			$botModule->sendSilentMessage($data->object->peer_id, $msg, $data->object->from_id);
		} else
